...
const abc = ids.filter((id) => id.includes("ABC"));
// console.log(abc);
// ----------------------------------------------------------
// INCLUDE EXERCISES
// ----------------------------------------------------------
// 1. It best to use INCLUDES when what the value is does not matter, simply its presence.
...
// 2. Checking between lists.
// Another thing that you might need to do in real life is check items between two arrays.
// Check if array B has any values that are also in array A.
...
const inter = A.filter((itemA) => B.map((itemB) => itemA.includes(itemB)));
console.log(inter);
